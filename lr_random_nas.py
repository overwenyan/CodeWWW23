...
if __name__ == '__main__':
    ...
    for lr in lr_hp_list:
        ...
        if hostname == 'fib':
            ...
            pass
        # avaliable_device_ids = GPUtil.getAvailable(order = 'first', limit = 8, maxLoad = 0.5, maxMemory = 0.2, includeNan=False, excludeID=[], excludeUUID=[])
        print('|------ START of learning rate ({}) optimization ------|'.format(lr))
        ...
        for tasks in task_index: #下面对同一种架构进行不同超参的并行 每个task对应在不同GPU上的工作
            with mp.Pool(processes=len(tasks),maxtasksperchild=1) as p:
                print('lr: {}, subtasks start!, tasks: {} on gpu-ids {}'.format(lr, tasks, assigned_device_ids))
                p.name = 'test'
                if args.data_type == 'implicit':# 装载任务
                    jobs = [[arch_single_list[i], num_users, num_items, [], valid_queue, test_queue, train_queue_pair, args, [float(lr), embedding_dim_list[i]], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks]
                else:
                    jobs = [[arch_single_list[i], num_users, num_items, train_queue, valid_queue, test_queue, args, [float(lr), embedding_dim_list[i]], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks] # 这一组task的lr都相同，其它随机，按照顺序
                rmse_list1 = p.map(get_single_model_performance, jobs) # 返回的数值是tasks的合计
                print('lr: {}, rmse_list1: {}'.format(lr, rmse_list1))
                print("task {} ended\n".format(tasks))
                # print() # 每一个lr对应一组rmse_list（实际上也可能是recall list）
                # run_one_model += 1
                # lr_result_list_dict
                recall_list1 = [rmse1[0] for rmse1 in rmse_list1]
                lr_result_list_dict[lr] += recall_list1
                p.close()
        print('lr end!',lr,'lr_result_list_dict', lr_result_list_dict)
        if len(lr_result_list_dict[lr]) < anchor_config_num:
            print('大小出问题')
        print('|------  END of learning rate ({}) optimization  ------|\n\n\n'.format(lr))
    
    
    print('lr_result_list_dict', lr_result_list_dict)
    info_json = json.dumps(lr_result_list_dict,sort_keys=False, indent=4, separators=(',', ': '))
    # 显示数据类型
    # print(type(info_json))
    f = open(logfilePath+'lrinfo'+str(args.file_id)+'.json', 'w')
