...
parser.add_argument('--anchor_num', type=int, default=16, help='size of anchor sample number')
parser.add_argument('--file_id', type=int, default=305, help='size of anchor sample number')
...
if __name__ == '__main__':
    ...
    for embsize in embs_hp_list:
        ...
        if hostname == 'fib':
            ...
            pass
        # avaliable_device_ids = GPUtil.getAvailable(order = 'first', limit = 8, maxLoad = 0.5, maxMemory = 0.2, includeNan=False, excludeID=[], excludeUUID=[])
        print('|------ START of embedding dim ({}) optimization ------|'.format(embsize))
        print("abc avaliable_device_ids: {}".format(avaliable_device_ids))
        if len(avaliable_device_ids) == 0:
            avaliable_device_ids = [3, 4] # 默认的两张卡，最少情况，程序一定要跑下去!不能中途中断！
        ...
        for tasks in task_index: #下面对同一种架构进行不同超参的并行 每个task对应在不同GPU上的工作
            with mp.Pool(processes=len(tasks),maxtasksperchild=1) as p:
                print('embedding_dim: {}, subtasks start!, tasks: {} on gpu-ids {}'.format(embsize, tasks, assigned_device_ids))
                p.name = 'test'
                if args.data_type == 'implicit':# 装载任务
                    jobs = [[arch_single_list[i], num_users, num_items, [], valid_queue, test_queue, train_queue_pair, args, [lr_list[i], int(embsize)], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks]
                else:
                    jobs = [[arch_single_list[i], num_users, num_items, train_queue, valid_queue, test_queue, args, [lr_list[i], int(embsize)], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks] # 这一组task的lr都相同，其它随机，按照顺序
                rmse_list1 = p.map(get_single_model_performance, jobs) # 返回的数值是tasks的合计
                print('embedding_dim: {}, rmse_list1: {}'.format(embsize, rmse_list1))
                print("task {} ended\n".format(tasks))
                # print() # 每一个lr对应一组rmse_list（实际上也可能是recall list）
                # run_one_model += 1
                # lr_result_list_dict
                recall_list1 = [rmse1[0] for rmse1 in rmse_list1]
                embedding_dim_result_list_dict[embsize] += recall_list1
                p.close()
        print('embsize end!',embsize,'embedding_dim_result_list_dict', embedding_dim_result_list_dict)
        if len(embedding_dim_result_list_dict[embsize]) < anchor_config_num:
            print('大小出问题')
        print('|------  END of embedding_dim ({}) optimization  ------|\n\n\n'.format(embsize))
    
    
    print('embedding_dim_result_list_dict', embedding_dim_result_list_dict)
    info_json = json.dumps(embedding_dim_result_list_dict,sort_keys=False, indent=4, separators=(',', ': '))
    # 显示数据类型
    # print(type(info_json))
    f = open(logfilePath+'embinfo'+str(args.file_id)+'.json', 'w')
    f.write(info_json)
